...
Coordinated multiple views (CMV) are a foundational design pattern in visual analytics, enabling users to examine data through multiple linked representations~\cite{roberts2007coordinated}. In spatial decision support, CMV architectures typically coordinate a map view with statistical summaries, parameter controls, and diagnostic displays, allowing analysts to observe how changes in one dimension propagate across others. Shneiderman's information-seeking mantra---\emph{overview first, zoom and filter, details on demand}~\cite{shneiderman1996eyes}---provides a complementary interaction framework that guides the progressive disclosure of spatial information. Within the cartographic domain specifically, Roth~\cite{roth2013taxonomy} developed an empirically-derived taxonomy of interaction primitives for map-based visualisation, distinguishing \emph{objective primitives}---what the user aims to accomplish (\emph{identify}, \emph{compare}, \emph{rank}, \emph{associate}, \emph{delineate})---from \emph{operator primitives}---the interactive functionality provided to achieve it (e.g., \emph{filter}, \emph{retrieve}, \emph{resymbolize}, \emph{overlay}, \emph{calculate}). This taxonomy, which builds on Yi et al.'s~\cite{yi2007interaction} intent-based interaction framework, provides a structured vocabulary for evaluating the interaction richness of map-based decision support systems. We use Roth's framework as an analytical lens for our platform's interaction design in Section~\ref{sec:roth-taxonomy}.
